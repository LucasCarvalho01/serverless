...
REDIS_OUTPUT_KEY = os.getenv("REDIS_OUTPUT_KEY", "lucassilva-proj3-output")
...
def load_pyfile():
    filepath = Path("/opt/usermodule.py")
...
def execute_dynamic_function(input, context):
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        usercode_last_update = datetime.datetime.now(pytz.timezone('America/Sao_Paulo')).strftime('%Y-%m-%d %H:%M:%S')
